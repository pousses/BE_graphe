...
public class Path {
    ...
    public static Path createFastestPathFromNodes(Graph graph, List<Node> nodes)
        ...
        if (!nodes.isEmpty()) {
            Node nPrecedent = nodes.getFirst();
            for (Node n : nodes) {
                if (n != nPrecedent) {
                    Arc result = null;
                    for (Arc a : nPrecedent.getSuccessors()) {
                        if (a.getDestination() == n) {
                            if (result == null) {
                                result = a;
                            }
                            else if (result.getMinimumTravelTime() >= a
        ...
        if (nodes.size() == 1) {
            solution = new Path(graph, nodes.getFirst());
        }
        else {
            solution = new Path(graph, arcs);
        }
        return solution;
    }
                ...
                if (n != nPrecedent) {
                    ...
                    for (Arc a : nPrecedent.getSuccessors()) {
                        if (a.getDestination() == n) {
                            ...
                            else if (result.getLength() >= a.getLength()) {
                                result = a;
                            }
                        }
                    }
    ...
    public boolean isValid() {
        ...
        if (this.isEmpty()) {
            ...
            result = true;
            Arc aPrece = this.arcs.getFirst();
            for (Arc a : this.arcs) {
                if (a == aPrece) {
                    if (a.getOrigin() != this.origin) {
                        result = false;
                    }
                }
                else {
                    if (a.getOrigin() != aPrece.getDestination()) {
                        result = false;
                    }
                }
                aPrece = a;
